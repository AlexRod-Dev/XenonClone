...
void Companion::Update()
{
	...
	if (spaceship)
	{
		...
		transformComponent->position.y = spaceship->GetPosition().y + offsetY;
...
void Companion::Fire()
{
	if (spaceship->IsFiring())
	{
		...
		{
			GameManager::GetInstance()->InstantiateProjectile<PlayerProjectile>(Vector2D(position.x + gunOffset.x, position.y + gunOffset.y), 1000, 12);
		}
		else if (currentMissileAugment == MissileAugment::MEDIUM)
		{
			GameManager::GetInstance()->InstantiateProjectile<PlayerProjectileMedium>(Vector2D(position.x + gunOffset.x, position.y + gunOffset.y), 1000, 12);
		}
		else if (currentMissileAugment >= MissileAugment::HEAVY)
		{
			GameManager::GetInstance()->InstantiateProjectile<PlayerProjectileHeavy>(Vector2D(position.x + gunOffset.x, position.y + gunOffset.y), 1000, 12);
		}
	}
}
